...
def plot2d(df, x, y, dire, n, heatmap_column,dataset_dir):
    """Generate 2D plots with optional heatmap coloring."""
    try:
        logger.info(f"Creating 2D plot {n} for {x} vs {y}")
        lincor = distance_correlation(df[x], df[y])
        log_data = np.log10(df[x]) if not np.any(df[x] <= 0) else df[x]
        logcor = distance_correlation(log_data, df[y])

        if abs(lincor) > 0.7 or abs(logcor) > 0.7:
            # Sanitize inputs and create directory
            ...
            fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(25, 8), dpi=300, sharey=True)
            fig.suptitle(f"{safe_x} vs {safe_y}", fontsize=16)
            ...
            # Log-scale plot
            ax1.scatter(log_data, df[y], c=df[heatmap_column] if heatmap_column else 'red',
                    cmap=cmap, norm=norm, s=30, alpha=0.7)
            ax1.set_title(f"Log Scale (DCOR: {logcor:.2f})")
            ax1.set_xlabel(f"log10({safe_x})" if not np.any(df[x] <= 0) else safe_x)

            # Linear-scale plot
            ax2.scatter(df[x], df[y], c=df[heatmap_column] if heatmap_column else 'red',
                    cmap=cmap, norm=norm, s=30, alpha=0.7)
            ax2.set_title(f"Linear Scale (DCOR: {lincor:.2f})")
            ax2.set_xlabel(safe_x)

            # Add colorbar if using heatmap
            if heatmap_column and norm:
                cbar = fig.colorbar(sm, ax=[ax1, ax2], pad=0.02)
                cbar.set_label(heatmap_column, rotation=270, labelpad=15)

            # Save plot
            filename = pathlib.Path(os.path.normpath(os.path.join(dire, f"2d_{safe_n}.png")))
            if len(str(filename)) > 100:
                ...
                filename = pathlib.Path(os.path.normpath(os.path.join(dire, f"2d_{hash_id}.png")))
            plt.savefig(filename, bbox_inches="tight")
            
            logger.info(f"Saved 2D plot to {filename}")
            save_completed_plot(safe_n)
        else:
            logger.info(f"Skipping 2D plot {n} - correlation coefficients are too low")
...
def plot3d(df, x, y, z, dire, n, heatmap_column,dataset_dir):
    ...
    try:
        ...
        if np.any(scope.values <= 0):
            data_variants = [scope] * 3
            labels = [(x, y, z)] * 3
        else:
            data_variants = [
                np.log10(scope[[x, y, z]].rename(columns={x: f"log_{x}"})),
                np.log10(scope[[x, y, z]].rename(columns={y: f"log_{y}"})),
                np.log10(scope[[x, y, z]].rename(columns={z: f"log_{z}"}))
            ]
            labels = [
                (r"$\log_{10}$"+x, y, z),
                (x, r"$\log_{10}$"+y, z),
        ...
        # Plot each variant in its subplot
        for ax, (data, (xl, yl, zl)) in zip(axes, zip(data_variants, labels)):
            ...
            if pca.explained_variance_ratio_[2] <= 1e-2:
                xx, yy = np.meshgrid(
                    np.linspace(data.iloc[:,0].min(), data.iloc[:,0].max(), 10),
        ...
        filename = pathlib.Path(os.path.normpath(os.path.join(save_dir, f"3d_{safe_n}.png"))) #{prefix}
        if len(str(filename)) > 100:
            logger.warning(f"Filename {filename} is too long, using hash instead")
            hash_id = hashlib.md5(safe_n.encode()).hexdigest()[:6]
            filename = pathlib.Path(os.path.normpath(os.path.join(save_dir, f"3d_{hash_id}.png"))) #{prefix}
...
def process_dataframe(df: pd.DataFrame,
                     ...
                     heatmap_column: str = "target",
                     include_base: bool = True) -> None:
    """
    ...
    try:
        ...
        for subset_name, col_regex in column_regex_config.items():
            try:
                ...
                if heatmap_column and heatmap_column not in subset_cols:
        ...
        for dataset_name, dataset_df in datasets:
            try:
                    ...
                    for (x, y) in itertools.permutations(numeric_cols, 2):
                        ...
                        if combo_key not in plotted_2d:
                            ...
                            if plot_condition(plot_name):
                                plot2d(dataset_df, x, y, plot2d_dir, plot_name, heatmap_column,dataset_dir)
                            else:
                                logger.info(f"Skipping 2D plot {plot_name} - columns not useful")
                            plotted_2d.add(combo_key)
                            plotted_2d.add((y, x))  # Add reverse to prevent mirror plots
                        else:
                ...
                if len(numeric_cols) >= 3:
                    ...
                    for combo in itertools.combinations(numeric_cols, 3):
                        ...
                        if combo_key not in plotted_3d:
                            ...
                            if plot_condition(plot_name):
                                plot3d(dataset_df, *combo, plot3d_dir, plot_name, heatmap_column, dataset_dir)
                            else:
...
if __name__ == "__main__":
    ...
    df_observed = df_numeric.filter(regex = r'(mag)|(erg)|^ID|z_|^RAdeg|^DECdeg$', axis=1)
    df_derived = df_numeric.drop(columns=[i for i in df_observed.columns if i!='z_best'])
    df_dict = {"Derived" : df_derived,"Observed": df_observed}
    for n,d in df_dict.items():
        # Fix 3: Verify column names before processing
        print("Columns in cleaned_numeric:", d.columns.tolist())
        print("Number of columns in cleaned_numeric:", len(d.columns.tolist()))

        # Fix 4: Update process_dataframe call

        CHECKPOINT_FILE = f"{n}_plots.txt"
